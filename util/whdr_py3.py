...
def compute_whdr(reflectance, judgements, delta=0.10):
    ...
    for c in comparisons:
        ...
        # l1 selects the pixel value at point 1 and takes the mean over all
        # color channels (-> greyscale), l2 does the same for point 2:
        l1 = max(1e-10, np.mean(reflectance[int(point1['y'] * rows),
                                            int(point1['x'] * cols), ...]))
        l2 = max(1e-10, np.mean(reflectance[int(point2['y'] * rows),
                                            int(point2['x'] * cols), ...]))

        # convert algorithm value to the same units as human judgements
        # now we label the reflectance comparison depending on the pixel values
